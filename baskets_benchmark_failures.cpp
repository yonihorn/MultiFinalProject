...
class OperationsList {
    ...
    class Operation {
        public:
        virtual void performOperation(BasketsQueue<u_int>* queue, u_int tid) = 0;
    };

    class InsertionOperation : public Operation {
        protected:
        u_int m_value;
        public:
        InsertionOperation(u_int value) : m_value(value) {};
        virtual void performOperation(BasketsQueue<u_int>* queue, u_int tid) {
            queue->enqueue(m_value, tid);
        }
    };

    class DeletionOperation : public Operation {
        public:
        virtual void performOperation(BasketsQueue<u_int>* queue, u_int tid) {
            queue->dequeue(tid);
        };
    ...
    void operateNextOperation(BasketsQueue<u_int>* queue, u_int tid) {
        m_operations[m_operations.size() - operationsLeft--]->performOperation(queue, tid);
    };

};


void queueOperationsWorker(BasketsQueue<u_int>* queue, OperationsList* opsList, u_int tid) {
...
u_int run(int nthreads, int ntrials, int noperations, double ratio, bool prefill, bool HTM_CAS) {
    ...
    for (u_int j = 0; j<ntrials; j++) {
        ...
        if (HTM_CAS) {
            queue = reinterpret_cast<BasketsQueue<u_int>*>(new BasketsQueueHTM<u_int>(failures_counters.data()));
        } else {
            queue = reinterpret_cast<BasketsQueue<u_int>*>(new BasketsQueueSTD<u_int>(failures_counters.data()));
        }
